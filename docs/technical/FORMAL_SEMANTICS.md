...
**Note:** Formal semantics validated through SMT solver analysis. Real-world validation in progress (TRL-4).
...
Using the **Z3 SMT Solver**, we have validated that for any given set of inputs $I$ within the logical model, there is **exactly one** valid output $O$ (at the logical model level).

* **No Race Conditions:** Logic is designed to be synthesized into parallel hardware gates (future work).  
* **No Jitter:** Execution is designed to be clock-locked to the FPGA oscillator (future work).

**Note:** Determinism is guaranteed at the logical model level. Real-world implementation may be affected by sensor errors, environmental factors, and hardware limitations. See `docs/LIMITATIONS.md` for full discussion.
